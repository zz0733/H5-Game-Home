...
(function (service) {
    ...
    */
    ...
    var TcpSocketService = /** @class */ (function () {
         ...
         * 构造
         */
        function TcpSocketService(module, serviceKind) {
            /**
             * 接收长度
             */
            this.m_wRecvSize = 0;
                    ...
                    while (_this.m_wRecvSize >= df.Len_Tcp_Head) {
                        ...
                        if (wMainCmd == df.MDM_KN_COMMAND) {
                            ...
                            if (wSubCmd == df.SUB_KN_DETECT_SOCKET) {
                                //回复心跳
                                _this.getServiceModule().getDelegate().sendPing(_this);
                            }
                            ...
                            _this.m_ServiceModule._MsgQueue.push(msg);
                        }
                        //处理长度
        ...
        TcpSocketService.prototype.socketConnectSuccess = function () {
            ...
            this.m_CurTime = egret.getTimer();
            //连接通知
            var delegate = this.m_ServiceModule.getDelegate();
            if (null != delegate) {
        ...
        TcpSocketService.prototype.getServiceModule = function () {
