namespace client {
    ...
    export class LogonLayer extends eui.UILayer {
         ...
         * 自动登录
         */
        private _isAutoLogon: boolean = false;

        /**
         * 手动输入
         */
        private _isHandInput: boolean = false;
        ...
        private onInitLayer(): void {
            ...
            setTimeout(() => {
                ...
                //无切换账号且勾选自动登录
                if (this._isAutoLogon && managers.FrameManager.getInstance().m_IsAuto) {
                    //自动登录
                    this._account = this._localStorage.account
                    this._password = this._localStorage.password;
